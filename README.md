...
Полный пайплайн агента: **capture → triage → package → [query · apply · digest · decide] → lint**.
Анализ — это не «эссе на почитать», а **действие, ответ или решение, привязанное к твоей цели**:
- **Query** — спрашиваешь → ответ из твоих знаний с цитатами;
- **Apply** — знание → протокол/эксперимент, который запускаешь;
- **Digest** — выжимка под активные цели + 1 шаг, поднимает забытое;
- **Decide** — развилка → рекомендация из базы со спорным;
- **Lint** — гигиена графа (противоречия, сироты, пробелы).

Все сценарии — в [USER-STORIES.md](./USER-STORIES.md).
