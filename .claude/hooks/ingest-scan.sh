...
if [ "${#new[@]}" -eq 0 ]; then
  ...
  echo "Проактивно (если Кристина не дала задачу): коротко предложи дайджест по базе (CLAUDE.md → Digest):"
  echo "  5 последних изученных тем · что давно не повторяла · что было бы интересно изучить (Scout под Фокус)."
  echo "Не навязывай — предложи одной строкой."
  exit 0
fi
