...
if [ "${#new[@]}" -eq 0 ]; then
  ...
  echo "Проактивно (если Кристина не дала задачу): предложи короткий дайджест —"
  echo "1 тема под «Сейчас в фокусе» из wiki/«Обо мне» + 1 забытое релевантное + 1 конкретный шаг."
  echo "Не навязывай — просто предложи одной строкой."
  exit 0
fi
